...
if ($user_usertype_current <= 3 && $_GET[user_id] != $_COOKIE[user]) { echo "<h1 class=\"heading_alert\">Permission Denied</h1><p>You do not have permission to view this page.</p>"; }

elseif ($user_usertype_current <= 3 && $_GET[user_id] == NULL) { echo "<h1 class=\"heading_alert\">Permission Denied</h1><p>You do not have permission to view this page.</p>"; }

else {

echo "<h2>Validate Expenses</h2>";
...
echo "<form action=\"index2.php?page=timesheet_expense_verify\" method=\"post\">";

echo "<table summary=\"List of expenses for all projects\">";
echo "<tr><td colspan=\"2\"><strong>Project</strong></td><td><strong>Date</strong></td><td><strong>Description</strong></td><td><strong>User</strong></td><td><strong>Value</strong></td><td><strong>Verified</strong></td><td><strong>Invoiced</strong></td></tr>";
...
while ($array = mysql_fetch_array($result)) {
	...
	if ($ts_expense_notes != NULL) { $rowspan = " rowspan=\"2\""; } else { $rowspan = ""; }
		...
		if ($proj_id_current != $proj_id) {	echo "<tr><td $rowspan $bg>".$proj_h2."</td>"; } else { $proj_id_current = $proj_id; }

		echo "<td $bg>".TimeFormat($ts_expense_date)."</td><td $bg><a href=\"index2.php?page=timesheet_expense_view&amp;ts_expense_id=$ts_expense_id\">".$ts_expense_desc."&nbsp;[$ts_expense_id]</a>";
		
		if ($user_usertype_current > 3 AND $ts_expense_invoiced < 1) { echo "&nbsp;<a href=\"index2.php?page=timesheet_expense_edit&amp;status=edit&amp;ts_expense_id=$ts_expense_id\"><img src=\"images/button_edit.png\" alt=\"Edit this entry\" /></a>"; }
		
		echo "</td><td $rowspan $bg><a href=\"index2.php?page=user_view&amp;user_id=$user_id\">$user_initials</a></td><td $rowspan $bg>".$ts_expense_vat_print."</td>";
		
		$expense_total = $expense_total + $ts_expense_vat;
		
		if ($user_usertype_current > 3 AND $ts_expense_verified == 0 AND $_GET[user_id] == "") { echo "<td $bg $rowspan><input type=\"checkbox\" value=\"$ts_expense_id\" name=\"ts_expense_verified[]\" /></td>"; } elseif ( $ts_expense_verified == 0 ) { echo "<td $rowspan $bg>No</td>"; } else { echo "<td $rowspan $bg><a href=\"index2.php?page=datebook_view_day&amp;time=$ts_expense_verified\">".TimeFormat($ts_expense_verified)."</a></td>"; }
		...
		echo "<input type=\"hidden\" name=\"ts_expense_id[]\" value=\"$ts_expense_id\" /><input type=\"hidden\" name=\"ts_expense_desc[]\" value=\"$ts_expense_desc\" /></td></tr>";
		
		if ($ts_expense_notes != NULL) {
			echo "<tr><td colspan=\"3\" $bg><span class=\"minitext\">Notes: ".PresentText($ts_expense_notes)."</span></td></tr>";
		}
		
	}

	echo "<tr><td colspan=\"5\"><strong>TOTAL</strong></td><td colspan=\"3\" style=\"text-align:right;\"><strong>".MoneyFormat($expense_total)."</strong></td></tr>";
	...
	if ($_GET[user_id] > 0) {
		...
		echo "<p>You have no outstanding expenses on the system.</p>";
	
	} else {
	
		echo "<p>There are no expenses to verify.</p>";
	
	}
}

if ($user_usertype_current > 3 AND $_GET[user_id] == NULL) {
echo "<tr><td colspan=\"8\"><input type=\"reset\" value=\"Clear\" />&nbsp;<input type=\"submit\" value=\"Verify expenses (".TimeFormat(time()).")\" /></td></tr>";
}

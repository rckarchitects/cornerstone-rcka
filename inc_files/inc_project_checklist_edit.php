...
echo "<h2>Project Checklist</h2>";
	...
	function CheckListItemUpdate ($proj_id) {
		
		global $conn;
		$proj_id = intval($proj_id);

		if (intval($_POST[item_id]) > 0 && $_POST[item_notes] != NULL OR $_POST[item_stage] != NULL) {
			$item_id = intval($_POST[item_id]);
			$item_notes = trim ( addslashes ($_POST[item_notes]) );
			$item_stage = intval($_POST[item_stage]);
			...
			AlertBoxInsert($_COOKIE[user],"Tasklist Item Updated","<p>Checklist item ref. $item_id within stage $item_stage updated.</p>",$item_id,0,0,$proj_id);
			
		}

	}
	
	function CheckListItemInsert($proj_id) {
		...
		if ($_GET[action] == "checklist_duplicate_item") {
				...
				AlertBoxInsert($_COOKIE[user],"Tasklist Item Duplicated","<p>Checklist item ref. $item_id within stage $item_stage duplicated.</p>",$item_id,0,0,$proj_id);
			
		}
		
	}
	
	function CheckListItemDelete($proj_id) {
		...
		if ($_GET[action] == "checklist_delete_item") {
				...
				AlertBoxInsert($_COOKIE[user],"Tasklist Item Deleted","<p>Checklist item ref. $item_id within stage $item_stage deleted.</p>",$item_id,0,0,$proj_id);
			
		}
		
	}

	function CheckListItemArray($proj_id,$group_id) {
	...
	$group_id = intval($group_id);
						...
						if (mysql_num_rows($result_checklist) > 0) {
							...
							if ($checklist_id > 0) {
								echo "&nbsp;<span class=\"minitext\">&nbsp;<a href=\"index2.php?page=project_checklist_edit&amp;proj_id=$proj_id&amp;&amp;group_id=$group_id&amp;action=checklist_duplicate_item&amp;item_id=$item_id \" onclick=\"return confirm('Are you sure you want to duplicate the checklist entry for \'$item_name\'?')\">[+]</a></span>";
							}
							
							// Exclude the delete button if this is the second time it appears
							if ($current_item == $item_id) {
								echo "<span class=\"minitext\">&nbsp;<a href=\"index2.php?page=project_checklist_edit&amp;proj_id=$proj_id&amp;group_id=$group_id&amp;action=checklist_delete_item&amp;checklist_id=$checklist_id \" onclick=\"return confirm('Are you sure you want to delete the checklist entry for \'$item_name\'?')\"><img src=\"images/button_delete.png\" alt=\"Delete Entry\" /></a></span>";
							}
							//}
	...
	CheckListItemUpdate ($proj_id) ;
	CheckListItemInsert ($proj_id) ;
	CheckListItemDelete($proj_id) ;

	StageTabs($group_id, $proj_id, "index2.php?page=project_checklist_edit&amp;proj_id=$proj_id","edit");
	
	CheckListItemArray($proj_id,$group_id);
